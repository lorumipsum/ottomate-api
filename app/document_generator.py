...
"""
    ...
    def generate_validation_report_with_tests(self, blueprint: Dict[str, Any], lint_result: Dict[str, Any], test_summary: Dict[str, Any] = None) -> str:
        """Generate validation_report.md with test results included."""

        # Generate base validation report
        base_report = self.generate_validation_report(blueprint, lint_result)
...
- **Errors:** {test_summary.get("errors", 0)} ⚠️
- **Success Rate:** {test_summary.get("success_rate", 0):.1f}%
- **Average Execution Time:** {test_summary.get("average_execution_time", 0):.3f}s

### Test Status
{"✅ All tests passing" if test_summary.get("success_rate", 0) == 100 else "⚠️ Some tests failing" if test_summary.get("failed", 0) > 0 else "ℹ️ Tests completed with mixed results"}
"""

            # Insert test section before the final separator
